...
export class RoundhoursComponent implements OnInit {
  ...
  constructor(
   public datePipe: DatePipe,
    public rhservice: RoundhoursService, public route: Router,private spinner: NgxSpinnerService) {
  }
  ...
  refreshList() {
    const me = this;
    me.roundHours = [];
    this.rhservice.roundHoursListbydate(this.date).toPromise().then(res => {
  ...
  editData(data) {
    ...
    this.rhservice.roundhourInfo.plant = data.plant;
    this.rhservice.roundhourInfo.rtype = data.rtype;
    this.rhservice.ptype = data.ptype;
    this.rhservice.roundhourInfo.r8to9 = data.r8to9;
    console.log("rtype",this.rhservice.roundhourInfo.rtype);

    console.log("8to9",this.rhservice.roundhourInfo.r8to9);
  ...
  OndateChange(val)
  {
    const me = this;
    me.roundHours = [];
    this.rhservice.roundHoursListbydate(val).toPromise().then(res => {
      const roundHours = res as Roundhours[];
